...
class ColetorTramitacoes(ColetorBase):
    def __init__(self, max_workers=5):
        super().__init__(tipo="tramitacoes")
        self.max_workers = max_workers
        self.caminho_saida = os.path.join(DIRETORIO_RAW, "tramitacoes.csv")
        self.tramitacoes_existentes = self._carregar_ids_existentes()
    ...
    def _obter_situacao_proposicao(self, id_proposicao, tentativas=10, tempo_max=600):
                ...
                if resp.status_code == 200:
                    ...
                    if dados:
                        ...
                        resultado = {
                            ...
                            "descricaoSituacao": ultima.get("descricaoSituacao", ""),
                            "dataHora": ultima.get("dataHora", "")
                        }
                        self._salvar_incremental(resultado)
                        print(f"[OK] {id_proposicao}: {resultado['descricaoSituacao']}")
                        return
                    else:
                        print(f"[OK] {id_proposicao}: sem tramitações registradas.")
                        return
                else:
    ...
    def baixar(self, df_proposicoes: pd.DataFrame):
        ...
        lista_ids = [i for i in lista_ids if i not in self.tramitacoes_existentes]

        print(f"🔎 Total de proposições: {len(df_proposicoes)}")
        print(f"✅ Já coletadas: {len(self.tramitacoes_existentes)}")
        print(f"⏳ Restantes para coleta: {len(lista_ids)}")
        print(f"🚀 Iniciando com {self.max_workers} workers...")

        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            futures = {executor.submit(self._obter_situacao_proposicao, id_prop): id_prop for id_prop in lista_ids}
